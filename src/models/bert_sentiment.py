...
베이스 모델: klue/bert-base  (한국어 BERT)
입력: 기사 제목 + 본문 앞 512 토큰
출력: 긍정(0) / 중립(1) / 부정(2) 3-class 분류

실험
────
Exp-1  klue/bert-base, epochs=5, lr=2e-5 (기본)
Exp-2  snunlp/KR-FinBert-SC 비교 (감성에 특화된 한국어 BERT)
...
def train_bert_sentiment(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # ── 체크포인트 저장 (에폭마다) ───────────────────
        ckpt_dir = os.path.join(OUTPUT_DIR, f'checkpoint_bert_sentiment_{label}_ep{epoch}')
    ...
    sample_mask = next(iter(test_loader))['attention_mask'][:1].to(DEVICE)
    with torch.no_grad():
        for _ in range(10): model(sample_ids, attention_mask=sample_mask)
        t0 = time.time()
        for _ in range(200): model(sample_ids, attention_mask=sample_mask)
