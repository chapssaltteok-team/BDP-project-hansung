...
수정 이력
──────────
- ImageDataset 경로 처리 수정 (Windows 백슬래시 → Linux 슬래시)
- num_workers=2 → num_workers=0 (Windows 호환)

실행 순서
──────────
1. python src/models/bert_sentiment.py
2. python src/models/image_classifier.py
3. python src/models/ensemble.py
...
def get_text_probs(df) -> np.ndarray:
...
class ImageDataset(Dataset):
    def __init__(self, df, image_map):
        ...
        for _, row in image_map.iterrows():
            ...
            url2path[row['url']] = os.path.join('data', 'images', filename)
...
def get_image_probs(df, image_map) -> np.ndarray:
...
# ── α 탐색 ───────────────────────────────────────────────────────────────────
def search_alpha(p_text, p_img, y_true, n_steps=21):
    """
    α ∈ [0.0, 1.0] 탐색 → val Macro-F1 최대화
...
def run_ensemble():
    ...
    for name, probs in [('BERT (텍스트)', val_text), ('ResNet (이미지)', val_img)]:
        ...
        print(f"  {name:<20} Acc={r['accuracy']:.4f}  "
    ...
    for name, probs in [('BERT 단독', test_text), ('ResNet 단독', test_img)]:
        ...
        results[name] = r
        print(f"  {name:<24} {r['accuracy']:>8.4f}  "
    ...
    print(f"\n  최적 가중치: α={best_alpha:.2f} (텍스트) / "
          f"{1-best_alpha:.2f} (이미지)")
